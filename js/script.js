...
var FormatBTTVEmotes = function () {
	...
	_createClass(FormatBTTVEmotes, [{
		...
		key: 'makePlaceHolder',
		value: function makePlaceHolder(emoteId) {
			return '$bttv(' + emoteId + ')$';
		}
	}, {
		key: 'replacePlaceholders',
		value: function replacePlaceholders(message) {
		...
		value: function formatMessage(message, emotesString) {
			...
			emotes.forEach(function (emote, index, emotes) {
				...
				if (index === 0) {
					...
					messageParts.push(_this.makePlaceHolder(emote.emote));
				}
...
new _vue2.default({
    ...
    computed: {
        ...
        showLoadNewer: function showLoadNewer() {
        ...
        showLoadOlder: function showLoadOlder() {
            if (this.loadingBottom) {
                return false;
            }
    ...
    ready: function ready() {
        ...
        this.$el.querySelectorAll('.hide').forEach(function (elem) {
    ...
    methods: {
        loadOlder: function loadOlder() {
            ...
            if (this.state === 'default') {
                ...
                (function () {
                    _this2.loadingBottom = true;
                    var date = _this2.logs[_this2.logs.length - 1].created_at;
                    var perPage = 100;

                    _this2.$http.get('chat-logs?page=1&starting-from=' + date + '&limit=' + perPage).then(function (response) {
                        _this2.loadingBottom = false;

                        if (response.data.data.length < perPage) {
                            _this2.moreResultsOlder = false;
                        }

                        response.data.data.forEach(_this2._proccessMessage);
                    });
                })();
            }
        },
        loadNewer: function loadNewer() {
            ...
            if (this.state === 'conversation') {
                (function () {
                    _this3.loadingTop = true;
                    var date = _this3.logs[0].created_at;
                    var perPage = 100;

                    _this3.$http.get('chat-logs?page=1&starting-from=' + date + '&limit=' + perPage + '&direction=newer').then(function (response) {
                        _this3.loadingTop = false;

                        if (response.data.data.length < perPage) {
                            _this3.moreResultsNewer = false;
                        }
                        ...
                        data.forEach(function (message) {
                            if (message.emotes !== null) {
                                message.message = _this3.twitchEmotes.formatMessage(message.message, message.emotes);
                            }

                            message.message = _this3.bttvEmotes.formatMessage(message.message);
                            message.message = htmlEntities(message.message);
                            message.message = _this3.bttvEmotes.replacePlaceholders(message.message);
                            message.message = _this3.twitchEmotes.replacePlaceholders(message.message);
                            message.highlight = message.id === _this3.highlight.id ? true : false;
                            message.message = message.message.linkify();

                            _this3.logs.unshift(message);
                        });
                    });
                })();
            }
        ...
        reset: function reset() {
            var loadDefault = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : true;
        ...
        default: function _default() {
            ...
            this.$http.get('chat-logs?page=' + ++this.page + '&starting-from=' + this.loadedTime + '&limit=' + perPage + '&direction=older').then(function (response) {
                _this4.loadingBottom = false;

                if (response.data.data.length < perPage) {
                    _this4.moreResultsOlder = false;
                }

                response.data.data.forEach(_this4._proccessMessage);
            });
        },
        search: function search() {
            ...
            this.$http.get('chat-logs/search?page=' + ++this.page + '&term=' + this.searchKeyword + '&limit=' + perPage).then(function (response) {
                _this5.loadingBottom = false;

                if (response.data.data.length < perPage) {
                    _this5.moreResultsOlder = false;
                }

                response.data.data.forEach(_this5._proccessMessage);
            });
        },
        conversation: function conversation(message) {
            var _this6 = this;
            ...
            this.$http.get('chat-logs/conversation?date=' + message.created_at).then(function (response) {
                _this6.loadingBottom = false;

                response.data.data.forEach(_this6._proccessMessage);
            });
        },
        _proccessMessage: function _proccessMessage(message) {
            if (message.emotes !== null) {
...
},{"./BTTVEmotes":1,"./TwitchEmotes":2,"vue":6,"vue-resource":5}],4:[function(require,module,exports){
